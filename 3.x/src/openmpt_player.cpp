...
const char *END_OF_SONG = "end_of_song";
...
void OpenMPTPlayer::_physics_process(float delta) { fill_buffer(); }
...
void OpenMPTPlayer::load(String filename) {
    ...
    // to `ModPlayer` if we're assuming a `Node` has a fixed location in the
    // heap.
    ...
    interactive = static_cast<openmpt::ext::interactive *>(
...
String OpenMPTPlayer::get_filename() { return filename; }

real_t OpenMPTPlayer::get_buffer_length() { return gen->get_buffer_length(); }
...
void OpenMPTPlayer::set_interpolation_filter(int32_t value) {
    ...
    try {
        int param =
            openmpt::module::render_param::RENDER_INTERPOLATIONFILTER_LENGTH;
...
void OpenMPTPlayer::fill_buffer() {
    ...
    if (num_frames > 0) {
        // Also sets the number of frames that will be passed to `push_buffer`
        // below
        ...
        try {
            for (;;) {
                int frames_rendered;
                {
                    ...
                    // `read_interleaved_stereo` doesn't seem to throw any
                    // exceptions
                    frames_rendered = module->read_interleaved_stereo(
                        SAMPLE_RATE,
                        num_frames,
                        reinterpret_cast<float *>(write.ptr()));
                }
...
void OpenMPTPlayer::_register_methods() {
    ...
    register_property<OpenMPTPlayer, int>(
        "tempo", &OpenMPTPlayer::set_tempo, &OpenMPTPlayer::get_tempo, 0);
    register_property<OpenMPTPlayer, int>(
        "speed", &OpenMPTPlayer::set_speed, &OpenMPTPlayer::get_speed, 0);
    register_property<OpenMPTPlayer, double>("tempo_factor",
                                             &OpenMPTPlayer::set_tempo_factor,
                                             &OpenMPTPlayer::get_tempo_factor,
                                             1.0);
    register_property<OpenMPTPlayer, double>("pitch_factor",
                                             &OpenMPTPlayer::set_pitch_factor,
                                             &OpenMPTPlayer::get_pitch_factor,
                                             1.0);
    register_property<OpenMPTPlayer, real_t>("buffer_length",
                                             &OpenMPTPlayer::set_buffer_length,
                                             &OpenMPTPlayer::get_buffer_length,
                                             DEFAULT_BUFFER_LENGTH);
    register_property<OpenMPTPlayer, bool>(
        "loop", &OpenMPTPlayer::set_loop, &OpenMPTPlayer::get_loop, false);
    register_property<OpenMPTPlayer, int32_t>(
        "interpolation_filter",
        &OpenMPTPlayer::set_interpolation_filter,
        &OpenMPTPlayer::get_interpolation_filter,
        0);

    register_signal<OpenMPTPlayer>(END_OF_SONG);
}
